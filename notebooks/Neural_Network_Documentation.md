...
```mermaid
flowchart TD
    subgraph "1️⃣ การเตรียมข้อมูล"
        A[Import Libraries<br/>นำเข้าไลบรารี] --> B[Load Data<br/>โหลดข้อมูลราคาน้ำมัน]
        B --> C[Data Exploration<br/>สำรวจข้อมูลเบื้องต้น]
        C --> D[Check Missing Values<br/>ตรวจสอบข้อมูลที่หายไป]
    end

    subgraph "2️⃣ Feature Engineering"
        D --> E[Create Lagged Features<br/>สร้าง features ราคาย้อนหลัง]
        E --> F[Create Moving Averages<br/>สร้างค่าเฉลี่ยเคลื่อนที่]
        F --> G[Create Target Variable<br/>สร้างตัวแปรเป้าหมาย]
    end

    subgraph "3️⃣ การเตรียมข้อมูลสำหรับ Training"
        G --> H[Select Features<br/>เลือก features ที่ใช้]
        H --> I[Train-Val-Test Split<br/>แบ่งข้อมูล 70-15-15]
        I --> J[Normalize Features<br/>Normalize ข้อมูล]
    end

    subgraph "4️⃣ การสร้างและ Train Model"
        J --> K[Create MLP Model<br/>สร้าง Neural Network]
        K --> L[Train Model<br/>Training โมเดล]
    end

    subgraph "5️⃣ การประเมินผล"
        L --> M[Make Predictions<br/>ทำนายผลลัพธ์]
        M --> N[Calculate Metrics<br/>คำนวณ MSE, RMSE, MAE, R²]
        N --> O[Visualize Results<br/>แสดงกราฟผลลัพธ์]
    end
...
def calculate_metrics(y_true, y_pred, dataset_name):
    ...
    # MSE (Mean Squared Error) - ค่าความคลาดเคลื่อนกำลังสองเฉลี่ย
    # คำนวณโดยยกกำลังสองของความต่าง แล้วหาค่าเฉลี่ย
    # ยิ่งต่ำยิ่งดี แต่มีหน่วยเป็นกำลังสอง (ดอลลาร์²)
    mse = mean_squared_error(y_true, y_pred)
    
    # RMSE (Root Mean Squared Error) - รากที่สองของ MSE
    # ดีกว่า MSE เพราะมีหน่วยเดียวกับข้อมูลต้นฉบับ (ดอลลาร์)
    # ให้น้ำหนักกับความผิดพลาดที่มากมากขึ้น
    rmse = np.sqrt(mse)
    
    # MAE (Mean Absolute Error) - ค่าความคลาดเคลื่อนสัมบูรณ์เฉลี่ย
    # ใช้ค่าสัมบูรณ์แทนการยกกำลังสอง ตีความง่ายกว่า RMSE
    # ตัวอย่าง: MAE = $1.50 หมายความว่าเฉลี่ยทำนายผิด $1.50
    mae = mean_absolute_error(y_true, y_pred)
    
    # R² (Coefficient of Determination) - สัมประสิทธิ์การตัดสินใจ
    # บอกว่าโมเดลอธิบายความผันแปรของข้อมูลได้เท่าไร
    # R² = 1.0 หมายถึง ทำนายถูก 100%
    # R² = 0.85 หมายถึง อธิบายข้อมูลได้ 85%
...
## 📊 คำอธิบายตัวชี้วัด (Metrics) อย่างง่าย

### 1. **MSE (Mean Squared Error)**
- **คือ:** ค่าเฉลี่ยของความผิดพลาดยกกำลังสอง
- **สูตร:** MSE = เฉลี่ย[(ค่าจริง - ค่าทำนาย)²]
- **ข้อดี:** ลงโทษความผิดพลาดที่มากมากขึ้น
- **ข้อเสีย:** มีหน่วยเป็นกำลังสอง (ดอลลาร์²) ตีความยาก

### 2. **RMSE (Root Mean Squared Error)**
- **คือ:** รากที่สองของ MSE
- **สูตร:** RMSE = √MSE
- **ข้อดี:** มีหน่วยเดียวกับข้อมูลต้นฉบับ (ดอลลาร์) ตีความง่าย
- **ตัวอย่าง:** RMSE = $2.50 หมายถึง ทำนายคลาดเคลื่อนเฉลี่ย $2.50

### 3. **MAE (Mean Absolute Error)**  
- **คือ:** ค่าเฉลี่ยของความผิดพลาดสัมบูรณ์
- **สูตร:** MAE = เฉลี่ย[|ค่าจริง - ค่าทำนาย|]
- **ข้อดี:** ตีความง่ายที่สุด ไม่ลงโทษความผิดพลาดมากเกินไป
- **ตัวอย่าง:** MAE = $1.20 หมายถึง ทำนายผิดเฉลี่ย $1.20

### 4. **R² (R-Squared)**
- **คือ:** เปอร์เซ็นต์ที่โมเดลอธิบายข้อมูลได้
- **ช่วงค่า:** 0 ถึง 1 (บางครั้งติดลบถ้าแย่มาก)
- **ตัวอย่าง:** 
  - R² = 0.95 = อธิบายได้ 95% (ดีมาก)
  - R² = 0.70 = อธิบายได้ 70% (พอใช้)
  - R² = 0.30 = อธิบายได้ 30% (แย่)

### เมื่อไหร่ใช้ตัวไหน?

| Metric | ใช้เมื่อ | ไม่ควรใช้เมื่อ |
|--------|---------|----------------|
| **MSE** | ต้องการลงโทษ outlier หนัก | ต้องการตีความผลง่าย |
| **RMSE** | ต้องการหน่วยเดียวกับข้อมูล | มี outlier มากเกินไป |
| **MAE** | ต้องการตีความง่าย ไม่ต้องการลงโทษ outlier | outlier สำคัญมาก |
| **R²** | ต้องการรู้ว่าโมเดล "ดี" แค่ไหน | เปรียบเทียบ dataset ต่าง scale |
...
| 9 | Evaluation Metrics | ✅ ถูกต้อง | MSE, RMSE, MAE, R² เป็น metrics มาตรฐาน |
...
6. **การประเมินผล** - ใช้ MSE, RMSE, MAE, R² ที่เป็นมาตรฐาน

### ความหมายของผลลัพธ์ที่ดี

- **MSE/RMSE/MAE ต่ำ** = ทำนายแม่นยำ ใกล้เคียงกับค่าจริง
- **R² สูง (ใกล้ 1)** = โมเดลอธิบายข้อมูลได้ดี
- **Test Metrics ใกล้เคียง Train Metrics** = ไม่ overfitting

---

*เอกสารนี้อธิบายการทำงานของ Neural Network สำหรับทำนายราคาน้ำมัน Brent โดยละเอียด หากมีข้อสงสัยเพิ่มเติม สามารถดูโค้ดจริงใน `Neural_Network.ipynb`*
